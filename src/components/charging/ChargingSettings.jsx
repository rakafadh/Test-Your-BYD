...
export default function ChargingSettings() {
  ...
  const handleAutoDelete = async () => {
    setDeleting(true);
    try {
      await autoDeleteOldRecords(autoDeleteDays);
      showSuccess(`Records older than ${autoDeleteDays} days have been deleted`);
      setShowDeleteModal(false);
    } catch (err) {
      showError('Failed to auto-delete records: ' + err.message);
    } finally {
      setDeleting(false);
    }
  };
  ...
  const getOldRecordsCount = () => {
    ...
    return chargingRecords.filter(record => 
  ...
  return (
    <div className="space-y-6 mt-4">
      ...
      <div className="card">
        ...
        <div className="space-y-4">
          ...
          <p className="text-gray-600">
            {autoDeleteEnabled 
              ? "Automatically delete charging records older than specified number of days to save storage space."
              ...
              {oldRecordsCount > 0 ? (
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                  ...
                  <p className="text-yellow-700 text-sm mt-1">
                    {oldRecordsCount} charging records are older than {autoDeleteDays} days and can be deleted.
                  </p>
                </div>
              ) : (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  ...
                  <p className="text-green-700 text-sm mt-1">
                    No charging records older than {autoDeleteDays} days found.
                  </p>
                </div>
              )}
            </>
          ...
          {autoDeleteEnabled && (
            <button
              ...
              disabled={!isOnline || oldRecordsCount === 0}
      ...
      <DeleteModal
        ...
        message={`Are you sure you want to delete ${oldRecordsCount} charging records older than ${autoDeleteDays} days? This action cannot be undone.`}
      />
    </div>
  );
}
