...
export default function TestDriveDashboard() {
  ...
  // Calculate pending: cars that went OUT but haven't come back IN
  const carStatusMap = new Map();
  
  // Sort by date_time to get chronological order
  const sortedTestDrives = [...testDrives].sort((a, b) => new Date(a.date_time) - new Date(b.date_time));
  
  // For each car (by police number), track the latest status
  sortedTestDrives.forEach(td => {
    if (td.police_number) {
      carStatusMap.set(td.police_number, td.status);
    }
  });
  
  // Count cars whose latest status is OUT (these are pending)
  const pending = Array.from(carStatusMap.values()).filter(status => status === 'OUT').length;
